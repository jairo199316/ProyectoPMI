...
public class CargoDAO {
     ...
     * @return una lista con los cargos registrados
     */
    public List<Cargo> listar(int proyectoId) {
        List<Cargo> listaCargos = new ArrayList<>();
        String consulta = "select id, nombre, descripcion, salario, horario, proyecto_id from Cargo where proyecto_id=" + proyectoId;
        Cursor temp = conex.ejecutarSearch(consulta);
        if (temp != null) {
            if (temp.moveToFirst()) {
                do {
